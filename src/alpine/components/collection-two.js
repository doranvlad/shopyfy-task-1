...
export const collectionFiltersTwo = () => ({
  isLoading: false,
  nextPageCount: 1,
  hasNextPage: false,
  floatFilters: false,
  activeCollection: {
    ...
    tempHandle: window.location.pathname.split("/").pop(),
  },
  facets_params: "",
  async init() {
  ...
  async renderCollection(collectionHandle) {
    ...
    const currentLayout = document.querySelector("[data-collection-content]");
    const currentBanner = document.querySelector("[data-collection-banner]");

    try {
      const response = await api.get(url, { responseType: "document" });
      const newLayout = response.querySelector("[data-collection-content]");
      const newBanner = response.querySelector("[data-collection-banner]");
  ...
  async changeCollection() {
    ...
    this.scrollToAnchor(document.body);
  },
});
